...
def runtest(mapfile, start, goal, verbose = True):
  ...
  # We can change the algorithms at here:
  MP = wAStar(boundary, blocks, weight = 1.2) # TODO: planner based on wA* algorithm
  # MP = RRTS(boundary, blocks, r=0.02, rewire_count=32) #TODO: planner based on RRT* algorithm
  # print("the boundary is:" + str(boundary))
  ...
  collision = False
  steps = len(path)
  # blocks = [[4.501, 4.501, 2.501, 5.499, 5.499, 3.499]]
  cc = collision_check(blocks)
  ...
  goal_reached = sum((path[-1]-goal)**2) <= 0.2
  print("there is collision in the path:" + str(collision))
